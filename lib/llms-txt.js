...
function renderLlmsTxt({ articles, baseUrl }) {
  ...
  lines.push(`- [Skill briefing](${baseUrl}/skill.md): Skill-shaped briefing for arriving agents — what AISS is, how to consume the corpus correctly, load-bearing gotchas.`);
  lines.push(`- [Agents guide](${baseUrl}/agents.md): \`AGENTS.md\`-convention entry point — surfaces, the search→fetch→cite pattern, install one-liner.`);
