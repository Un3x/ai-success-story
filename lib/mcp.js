...
function createMcpServer({ corpus, getBaseUrl, submissions, submitToken, adminToken, telemetry, caller }) {
  const callerBucket = caller === 'internal' ? 'internal' : 'unattributed';
  ...
  function withCounter(name, handler) {
    return async (...args) => {
      let result;
      let threw = false;
      try {
        ...
        if (telemetry && typeof telemetry.recordMcpCall === 'function') {
          const ok = !threw && !(result && result.isError === true);
          try { telemetry.recordMcpCall({ tool: name, ok, caller: callerBucket }); } catch (_) { /* noop */ }
        }
      }
    };
  }
  ...
  server.registerTool(
    ...
    withCounter('search_stories', async ({ situation, tags, limit }) => {
  ...
  server.registerTool(
    ...
    withCounter('fetch_story', async ({ slug, parts }) => {
  ...
  if (submissions) {
    ...
    server.registerTool(
      ...
      withCounter('submit_story', async ({ token, frontmatter, body }) => {
    ...
    server.registerTool(
      ...
      withCounter('submission_status', async ({ submission_id }) => {
    ...
    server.registerTool(
      ...
      withCounter('list_pending', async ({ admin_token }) => {
    ...
    server.registerTool(
      ...
      withCounter('approve_pending', async ({ submission_id, admin_token }) => {
    ...
    server.registerTool(
      ...
      withCounter('reject_pending', async ({ submission_id, admin_token, reason }) => {
