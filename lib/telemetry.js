...
function createTelemetry({
  ...
  function recordMcpCall({ tool, ok, caller }) {
    const t = typeof tool === 'string' && tool.length > 0 ? tool : 'unknown';
    const c = caller === 'internal' ? 'internal' : 'unattributed';
    const byTool = state.mcp.by_tool;
    if (!byTool[t]) byTool[t] = { ok: 0, err: 0 };
    if (!byTool[t].by_caller) byTool[t].by_caller = {};
    if (!byTool[t].by_caller[c]) byTool[t].by_caller[c] = { ok: 0, err: 0 };
    if (ok) {
      byTool[t].ok += 1;
      byTool[t].by_caller[c].ok += 1;
    } else {
      byTool[t].err += 1;
      byTool[t].by_caller[c].err += 1;
    }
